<script lang="ts">
    ...
    export let switchSrc: string
    export let switchAlt: string
    export let style: string = ""

    let switched: boolean = false
    let element
    onMount(() => element.setAttribute("tabindex", "0"))
</script>

<div class="parent" style={style} bind:this={element} on:mouseover={() => switched = true}
     on:mouseleave={() => switched = false} on:focus={() => switched = true} on:blur={() => switched = false}>
    <img class="image" src={src} alt={alt} draggable="false"/>
    <img class="image switch" class:show={switched} src={switchSrc} alt={switchAlt} draggable="false"/>
